...
| 2026-04-15 | P5.7: Gates 4-6 (provenance-continuity, release-age, scope-boundary) + dep cache + background fetcher | Wrote `gates/provenance-continuity.js` (Gate 4, WARN): history-driven, filters self-entry, SKIPs when no prior has ever used OIDC (nothing to regress from), ALLOWs when incoming continues the chain, WARNs only when at least one prior version had provenance and the incoming does not — no OSS tool currently flags Sigstore attestation regression, this is a genuine differentiator. Wrote `gates/release-age.js` (Gate 5, WARN): threshold from `config.releaseAgeHours` (default 72), clock via `input.config._nowMs` for determinism, clamps future-dated releases to age 0 (anti-forgery clock-skew defense), `ageHours >= threshold → ALLOW`, prerelease versions (lexical `-` after major.minor.patch) exempt to match npm 11.10 `min-release-age` semantics. Wrote `witness/dep-cache.js` + schema migration in `witness/db.js` — new `dep_first_publish` table (pkg PK, first_publish ISO, status CHECK IN ('ok','vanished','error'), cached_at, attempts). `DepCache` class with `lookup/recordOk/recordVanished/recordError`, injectable clock via `now` function, only `error` honors `errorTtlMs` (5-min negative TTL); `ok`/`vanished` are forever because npm's first-publish timestamp is immutable. All writes route through `sqlNowFrom(this._now())` so mocked clocks actually advance the stored timestamps. Wrote `proxy/dep-fetcher.js`: single-worker background pool, `deriveFirstPublish` prefers `time.created`, falls back to min version time skipping `created`/`modified` meta keys. Dedup via `inflight + pending` sets, drops excess beyond `maxQueueSize=100`, throttle via `await sleep(intervalMs=200)` between fetches for ~5 rps. `stop()` clears queue, awaits in-flight. Wrote `gates/scope-boundary.js` (Gate 6, WARN/BLOCK) — **the keystone ChainGate gate**. Composes three signals: (A) NEW runtime dep not in any prior version UNION, (B) `has_install_scripts=1/true`, (C) the new dep's own first-publish < 24h ago via dep-cache. `A+B → WARN`, `A+B+C → BLOCK`. Self-loop guard: a package listing itself as a dep is skipped. On any novel-dep call, enqueues cache fills for ALL new deps (not just the first) so subsequent observations have full signal-C coverage. `config._nowMs` injectable for determinism. Pattern — async-in-sync via persistent cache + background worker pool — solves the core architectural tension (gate runner is synchronous inside a transaction, but dep-age needs network). First observation fails open to WARN; second observation within cache lifetime catches the threat. Updated `gates/index.js` DEFAULT_GATE_MODULES to `[contentHash, depStructure, publisherIdentity, provenanceContinuity, releaseAge, scopeBoundary]`; added `services` param to `createGateRunner` that gets merged onto every input passed to modules. Updated `proxy/server.js` to construct `DepCache` + `createDepFetcher` when the witness DB is present, inject `lookupDepFirstPublish` / `enqueueDepLookup` into the gate runner, and drain the fetcher during `server.close` before closing the DB. Tests: `test/witness/dep-cache.test.js` 11/11, `test/proxy/dep-fetcher.test.js` 11/11, `test/gates/provenance-continuity.test.js` 12/12, `test/gates/release-age.test.js` 17/17, `test/gates/scope-boundary.test.js` 16/16. Updated runner `DEFAULT_GATE_MODULES` assertion to accommodate six modules. **Full suite 229/229 green**. **Smoke A (real npm, fresh DB)**: started proxy on :16177, fetched axios packument — 132 baselines recorded, all ALLOW, 7 entries in `gates_fired` per version (first-seen + six real gates, scope-boundary SKIP on empty history, release-age ALLOW on all, others SKIP on no-prior). **Smoke B (real npm, dep-fetcher)**: invoked `createDepFetcher` against real registry with `follow-redirects` / `form-data` / `proxy-from-env` / bogus name — `follow-redirects` wrote cache row `ok 2012-11-15T22:23:37.885Z` derived from upstream packument, confirming the end-to-end background path works. (The smoke script stop()s immediately which drops the queue — in the real proxy path, enqueue is never followed by immediate stop, so the worker drains naturally at 5 rps.) |
...
| 2026-04-22 | Per-major provenance semantics landed | Commits 9a2defd through 5340a6d. axios@1.14.1 BLOCKs on full production history with escalators [new_domain, privacy, machine_to_human]. Resolves cross-major contamination. |
